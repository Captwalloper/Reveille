...
namespace AdventureWorks.VoiceCommands
{
    ...
    //public interface ICortana
    //{
    //    void LaunchAppInForeground();
    //    void OnTaskCanceled(IBackgroundTaskInstance sender, BackgroundTaskCancellationReason reason);
    //    void OnVoiceCommandCompleted(VoiceCommandServiceConnection sender, VoiceCommandCompletedEventArgs args);
    //    void Run(IBackgroundTaskInstance taskInstance);
    //    Task SendCompletionMessageForCancellation(string phraseParam);
    //    Task SendCompletionMessageForDestination(string phraseParam);
    //    Task ShowProgressScreen(string message);
    //    Task<string> DisambiguateTrips(IEnumerable<string> phraseParams, string disambiguationMessage, string secondDisambiguationMessage);
    //}
    ...
    public sealed class AdventureWorksVoiceCommandService : IBackgroundTask
    {
        ...
        {
            const string phraseParamName = "destination";
            ...
            if ( triggerDetails != null && triggerDetails.Name == typeof(AdventureWorksVoiceCommandService).ToString() ) //AdventureWorksVoiceCommandService
            {
                try
                {
                    ...
                    switch (voiceCommand.CommandName)
                    {
                        case "whenIsTripToDestination":
                            var destination = voiceCommand.Properties[phraseParamName][0];
                            await SendCompletionMessageForDestination(destination);
                            break;
                        case "cancelTripToDestination":
                            var cancelDestination = voiceCommand.Properties[phraseParamName][0];
        ...
        private async Task SendCompletionMessageForCancellation(string destination)
        {
            ...
            //string progressScreenString = string.Format( cortanaResourceMap.GetValue("ProgressLookingForTripToDest", cortanaContext).ValueAsString, destination);
        ...
        /// <returns></returns>
        private async Task<string> DisambiguateTrips(IEnumerable<string> trips, string disambiguationMessage, string secondDisambiguationMessage)
        {
        ...
        private async Task SendCompletionMessageForDestination(string destination)
        {
            ...
            //string loadingTripToDestination = string.Format( cortanaResourceMap.GetValue("LoadingTripToDestination", cortanaContext).ValueAsString, destination);
            ...
            //    // periodically refreshes the phrase list so it's likely to be in sync is ideal.
            //    // This is unlikely to occur for this sample app, however.
            //    string foundNoTripToDestination = string.Format( cortanaResourceMap.GetValue("FoundNoTripToDestination", cortanaContext).ValueAsString, destination);
